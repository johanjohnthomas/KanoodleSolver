...
- Entry: the raised desk perspective is a start/resume screen, with a single centered moss Play button. Pieces and puzzle controls are inactive here; the physical drawer and room settings remain usable. Play closes the drawer and uses the existing camera transition into the upright overhead game. Desk view returns to this screen without resetting the board. A 2D shortcut and WebGL fallback enter the accessible game directly.
- Spoiler-free check: Check solvability reports only whether the current layout admits a completion. It never places pieces, reveals a hint/solution, recommends removals or changes Undo history. It clears prior recovery highlights. Keep results in the existing live status region and disable puzzle actions while checking.
...
The raised desk camera is at (0, 12.5, 24), looking at (0, -0.5, -1.5), fitting 28 world units wide / 28 tall. Overhead is at (0, 32, 4), looking at (0, 0, 0), fitting 21.5 × 16 with a small player-side tilt to avoid a camera pole crossing. View changes damp position, target and field of view at rate 8; reduced motion switches immediately. Initial rendering starts at the raised desk without an entrance sweep. Play is a 56px-high, 20px-label moss button with a Phosphor play icon, 12px radius and existing offset shadow; its overlay passes pointer events through everywhere else so the drawer remains discoverable. It gives press feedback and disappears as the camera enters the game, with no extra idle animation. Desktop stage height is clamp(620px, 75vh, 840px); phone controls reserve 145px above the room canvas (220px below 375px).
